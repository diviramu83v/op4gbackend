...
class CompletesDecoderTest < ActiveSupport::TestCase
  setup do
  ...
  describe 'fixture' do
    ...
    test 'connection to decoded_uid records' do
      assert_includes @decoding.decoded_uids, decoded_uids(:completes_decoder)
      assert_includes @decoding.decoded_uids, decoded_uids(:third)
    end
  end

  describe 'methods' do
    describe '#update_onboardings_close_out_reason' do
      setup do
        @decoding = completes_decoders(:standard)
        @close_out_reason = close_out_reasons(:rejected)
      end

      it 'should update the onboardings close out reason' do
        @decoding.update_onboardings_close_out_reason(@close_out_reason.id)

        assert_equal @decoding.onboardings.first.close_out_reason.title, 'Test'
        assert_equal @decoding.onboardings.first.close_out_reason.category, 'rejected'
        assert_equal @decoding.onboardings.last.close_out_reason.title, 'Test'
        assert_equal @decoding.onboardings.last.close_out_reason.category, 'rejected'
      end
    end
