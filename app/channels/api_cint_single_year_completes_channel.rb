...
class ApiCintSingleYearCompletesChannel < ApplicationCable::Channel
  def subscribed # rubocop:disable Metrics/AbcSize
    Rails.logger.debug 'Subscribed to the API Cint single year completes channel.'
  ...
  def unsubscribed
    Rails.logger.debug 'Unsubscribed from the API Cint single year completes channel.'
  end
  ...
  def add_totals_to_hash
    @completes_by_month.map do |date, completes|
      ...
      revenue = Onboarding.cint_completes_revenue_for_month(month: Date::MONTHNAMES[date.second], year: date.first)
      revenue = revenue.to_f
      payout = Onboarding.cint_completes_payout_for_month(month: Date::MONTHNAMES[date.second], year: date.first)
      payout = payout.to_f
